...
void Warlock::learnSpell(ASpell * aspell){
    if (aspell){
        this->spell.learnSpell(aspell);
        // std::map<std::string, ASpell *>::iterator it;
        // it = this->book.find(aspell->getName());
        // if(it == this->book.end())
        //     this->book.insert(std::make_pair(aspell->getName(), aspell));
    }
};

void Warlock::forgetSpell(std::string const name){
    this->spell.forgetSpell(name);
    // std::map<std::string, ASpell *>::iterator it;
...
bool isNULL(T const &t){
    return reinterpret_cast<const char*>(&t) == NULL;
}

void Warlock::launchSpell(std::string name, ATarget const & atarget){
    ASpell *aspell = this->spell.createSpell(name);
    if(aspell)
