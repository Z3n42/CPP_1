...
Warlock::~Warlock(){
    // if(!this->spells->book.empty()){
    //     std::map<std::string,ASpell*>::iterator it;
    //     for(it = this->spells->book.begin(); it != this->spells->book.end(); it++)
    //         delete (*it).second;
    // }
    // this->spells;
    // delete this->spells;
...
void Warlock::launchSpell(std::string const name, ATarget &target){
    ...
    // it = this->book.find(name);
    // if (it != book.end())
    this->spells.createSpell(name)->launch(target);
};