...
/*   Updated: 2022/02/08 14:20:50 by fsilva-f         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	zoom(t_hook *p_hook, int keycode)
{
	...
	factor = 1;
	if (keycode == 0x7D || keycode == 5)
		factor = 0.9;
	else if (keycode == 0x7E || keycode == 4)
		factor = 1.1;
...
int	key_press(int keycode, t_hook *param_hook)
{
	...
	else if (keycode == 0x7D || keycode == 0x7E)
		zoom(param_hook, keycode);
	else if (keycode == 0x7B || keycode == 0x7C)
