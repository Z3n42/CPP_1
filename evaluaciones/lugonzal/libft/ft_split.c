/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_split.c                                         :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: lugonzal <lugonzal@student.42urduliz.com>  +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/05/28 14:16:55 by lugonzal          #+#    #+#             */
/*Updated: 2021/05/28 14:16:57 by lugonzal               ###   ########.fr    */
/*                                                                            */
/* ************************************************************************** */
#include <stdlib.h>
#include "libft.h"

static char	**ft_frlloc(char **tab, int j)
{
	int	i;

	i = 0;
	while (j > i)
	{
		free(tab[i]);
		i++;
	}
	free(tab);
	return (NULL);
}

static int	ft_strc(const char *str, char c)
{
	int	i;

	i = 0;
	while (str[i] != c && str[i])
		i++;
	return (i);
}

static int	ft_tab_len(char const *s, char c)
{
	int	i;
	int	row;

	row = 0;
	i = 0;
	while (s[i])
	{
		if (s[i] == c && s[i - 1] != c && s[i - 1] != 0)
			row++;
		if (s[i + 1] == '\0' && s[i] != c)
			row++;
		i++;
	}
	return (row);
}

static char	**ft_handle_tab(char const *str, char c, char **tab, int k)
{
	int	j;
	int	i;

	i = 0;
	j = 0;
	while (str[i] && j < k)
	{
		if (str[i] != c)
		{
			tab[j] = (char *)malloc(sizeof (char) * (ft_strc(&str[i], c)) + 1);
			if (tab[j] == NULL)
				return (ft_frlloc(tab, j));
			tab[j] = ft_substr(str, i, ft_strc(&str[i], c));
			j++;
		}
		while (str[i] != c && str[i])
			i++;
		i++;
	}
	tab[j] = NULL;
	return (tab);
}

char	**ft_split(char const *s, char c)
{
	int		j;
	char	**tab;

	if (!s || !c)
		return (NULL);
	j = ft_tab_len(s, c);
	tab = (char **)malloc((j + 1) * (sizeof(char *)));
	if (!tab)
		return (NULL);
	return (ft_handle_tab(s, c, tab, j));
}
