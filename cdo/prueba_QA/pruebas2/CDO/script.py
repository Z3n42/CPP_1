...
def web_test(data, args, flag, argflag):
    ...
    ret = []
    for value in data:
        if value[flag][0] == argflag[0]:
            try :
                title = selena(argflag[0])   
...
class Qa(object):
    ...
    def select_flag(self):
        ...
        if self.args.alpha_two_code:
            argflag = self.args.alpha_two_code
            flag = "alpha_two_code"
            self.ret = simple_get(self.data, self.args, flag, argflag)
        if self.args.domains:
            argflag = self.args.domains
            flag = "domains"
            self.ret = list_get(self.data, self.args, flag, argflag)
        if self.args.state:
            argflag = self.args.state
            flag = "state-province"
            self.ret = simple_get(self.data, self.args, flag, argflag)
        if self.args.web_pages:
            argflag = self.args.web_pages
            flag = "web_pages"
...
# Argument parsing
def parser():
    parser = argparse.ArgumentParser(description='Filter University')
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument('-n', '--name', nargs='+', help='Insert a "name" of a University')
    group.add_argument('-a', '--alpha_two_code', nargs='+', help='Insert "alpha_two_code" of a University')
    group.add_argument('-d', '--domains', nargs='+', help='Insert "domain" of a University')
    group.add_argument('-s', '--state', nargs='+', help='Insert the name of the "state" of a University')
    group.add_argument('-w', '--web_pages', nargs='+', help='Insert the url of the "web_page" of a University')
    group.add_argument('-c', '--country', nargs='+', help='Insert the name of the "country" of a University')
